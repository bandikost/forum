...
//Импорт серий из базы данных
  ...
  const [isVisible, setIsVisible] = useState(true); // Set to true by default or load from local storage
  
  useEffect(() => {
    const db = getDatabase();
    const objects3Ref = ref(db, "objects3");

    // Use 'on' to create a real-time listener
    const unsubscribe = on(objects3Ref, (snapshot) => {
      const data = snapshot.val();
      setObjects3(data);
      // Assuming isVisible is stored in objects3
