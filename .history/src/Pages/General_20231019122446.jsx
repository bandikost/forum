...
import { getDatabase, ref, onValue, oт } from 'firebase/database';
...
//Импорт серий из базы данных
  ...
  const [isVisible, setIsVisible] = useState(() => {
    ...
    return storedIsVisible ? JSON.parse(storedIsVisible) : true; // Set to true if not stored
  });
  
  const handleImageCheckboxChange = () => {
    setIsVisible((prevVisibility) => {
      const updatedVisibility = !prevVisibility;
      // Store isVisible state in local storage
...
useEffect(() => {
  const fetchData = (snapshot) => {
    ...
    setObjects3(data);
    // Update the local state with the global visibility state from Firebase
    setIsVisible(data.isVisible); // Assuming the visibility state is stored in 'isVisible' property
  };

  onValue(objects3Ref, fetchData);

  return () => {
    // Unsubscribe from the Firebase Realtime Database when the component unmounts
    off(objects3Ref, fetchData);
  };
}, []);
