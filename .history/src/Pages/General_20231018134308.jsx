...
export const General = ({ id, showImage, i}) => {
...
const [objects, setObjects] = useState({});
  ...
  const array = [/* your data as an array */];
  for (let i = 0; i < array.length; i++) {
    // your code using 'i'
  }
  const [newValues, setNewValues] = useState({});

// Inside the useEffect where you fetch data from the database
onValue(objects3Ref, (snapshot) => {
  const data = snapshot.val();
  setObjects3(data);
});
{objects3 && objects3[`isVisible${i}`] && (
  <div style={{ display: "contents", margin: "0 auto 0" }}>
    <img width={100} src={objects3[`imageUrl${i}`]} alt="Sold" />
  </div>
)}
    ...
    <div className={s.s}>
        ...
        <Link className={s.ids} to={`/page/id-3`}>
          <img
            ...
            src={objects3[`imageUrl1`]}
            width={"230px"}
            height={"190px"}
            style={{ borderRadius: "5px" }}
          />
          <p className={s.opacity}>{objects3 && objects3.name1}</p>
          {showImage && <img className={s.soldimg} src={sold} alt="" />}
