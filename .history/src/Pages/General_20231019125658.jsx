...
//Импорт серий из базы данных
  ...
  const useObjectsEffect = (objectName, setObject, setIsVisible) => {
    useEffect(() => {
      const db = getDatabase();
      const objectRef = ref(db, objectName);
      const unsubscribe = onValue(objectRef, (snapshot) => {
    ...
    }, [objectName, setObject, setIsVisible]);
  };
  // Usage
  useObjectsEffect("objects3", setObjects3, setIsVisible);
  useObjectsEffect("objects", setObjects, setIsVisible);
  useObjectsEffect("objects2", setObjects2, setIsVisible);
  useObjectsEffect("objects4", setObjects4, setIsVisible);
  useObjectsEffect("objects5", setObjects5, setIsVisible);
  useObjectsEffect("objects6", setObjects6, setIsVisible);
...
if (!objectId) {
  ...
  src={isVisible ? discount : objects2[`imageUrl${i}`]}
  width={"230px"}
  height={"190px"}
  style={{
    borderRadius: "5px",
