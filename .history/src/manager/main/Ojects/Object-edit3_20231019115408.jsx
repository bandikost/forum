...
export const GeneralEdit3 = () => {
  ...
  const handleImageCheckboxChange = () => {
    setNewValues(prevState => {
      const updatedState = !prevState.isVisible;
      // Store isVisible state in local storage
      localStorage.setItem('isVisible', JSON.stringify(updatedState));
      return {
        ...prevState,
        isVisible: updatedState
      };
    });
  };
  ...
  useEffect(() => {
    ...
    const isVisibleRef = ref(db, "isVisible"); // Change "isVisible" to your Firebase path
    onValue(isVisibleRef, (snapshot) => {
      const isVisibleFromFirebase = snapshot.val();
      setNewValues((prevValues) => ({
        ...prevValues,
        isVisible: isVisibleFromFirebase || false, // Set to false if not found in Firebase
      }));
    });
  }, []);
