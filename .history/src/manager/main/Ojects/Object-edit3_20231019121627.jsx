...
export const GeneralEdit3 = () => {
  ...
  const handleImageCheckboxChange = () => {
    ...
    // Update the visibility state in the Firebase Realtime Database
    const db = getDatabase();
    const isVisibleRef = ref(db, 'isVisible');
    set(isVisibleRef, !isVisible); // Note: You should set the updated visibility state here
  };
  
  const [newValues, setNewValues] = useState({
    isVisible: objects3.isVisible || true,
  });
  useEffect(() => {
    const db = getDatabase();
    const isVisibleRef = ref(db, 'isVisible');
