...
export const GeneralEdit3 = () => {
  ...
  const [showImage, setShowImage] = useState(false); // Set the initial state to false

  const handleImageCheckboxChange = () => {
    console.log("Checkbox state before toggle:", showImage);
    setShowImage(!showImage); // Toggle the value of showImage
    console.log("Checkbox state after toggle:", showImage);
  };
  ...
  return (
    <>
      ...
      />{" "}Показывать?
      {showImage && objects3 && objects3[`imageUrl${i}`] && (
        <div style={{ display: "contents", margin: "0 auto 0" }}>
          <img width={100} src={sold} alt="Sold" />
        </div>
        )}
</label>
