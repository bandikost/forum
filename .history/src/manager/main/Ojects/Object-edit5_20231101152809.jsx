...
export const GeneralEdit5 = () => {
  ...
  const [newValues, setNewValues] = useState({
    isVisible5: objects5.isVisible5 || false,
    isSold5: objects5.isSold5 || false,
  });
  ...
  const handleSoldCheckboxChange = () => {
    const db = getDatabase();
    const objects5Ref = ref(db, "objects5"); // Assuming 'objects3' is the correct reference in your database
    set(objects5Ref, { ...objects5, isSold5: !newValues.isSold5 }); // Update the 'isVisible' property
  };
  ...
  useEffect(() => {
    if (objects5.name1) {
    document.title = objects5.name1 + " - Редактор";
    }
  }, [objects5.name1]);
          ...
          style={{position: "relative", top: "60px", margin: "0 auto 0"}}>
            {[1].map((i) => (
              <div key={i}  className="table-objects5" style={{display: "flex",
                ...
                >
                {objects5 && convertIdentifiersToTags(objects5?.[`minimal${i}`])}
                </textarea>
                </label>
                <label style={{ margin: "0px 50px" }}>
...
={(e) => handleImageUpload(e, i)} 
                ...
                <label style={{marginTop: "430px"}}>
                <input
                type="checkbox"
                checked={newValues.isVisible5}
