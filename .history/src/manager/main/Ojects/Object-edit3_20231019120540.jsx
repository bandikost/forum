...
export const GeneralEdit3 = () => {
  ...
  const handleImageCheckboxChange = () => {
    // Update isVisible in the Realtime Database
    const db = getDatabase();
    const isVisibleRef = ref(db, 'isVisible');
    set(isVisibleRef, !isVisible);
  };
  
  const [newValues, setNewValues] = useState({
    isVisible: false, // Set the initial state of isVisible to true or false based on your requirements
  });
  
  useEffect(() => {
    const db = getDatabase();
    const isVisibleRef = ref(db, 'isVisible');
    onValue(isVisibleRef, (snapshot) => {
      const isVisibleFromDatabase = snapshot.val();
      // Handle visibility changes here (e.g., update UI, etc.)
      // You might set this value to your local state if you need it for any other purposes.
    });
  }, []);
...
={(e) => handleImageUpload(e, i)} 
        ...
        <input
          ...
          onChange={() => {
            newValues.isVisible = !newValues.isVisible; // Toggle isVisible
          }} />
        {" "}
