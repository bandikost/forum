...
export const GeneralEdit1 = () => {
  ...
  const convertTagsToIdentifiers = (text) => {
    return text.replace(/\{([^}]+)\}/g, (match, tag) => `#TAG${tag}#`);
  };

  const convertIdentifiersToTags = (text) => {
  ...
  const handleFormSubmit = (event) => {
    ...
    // Replace tags with unique identifiers before saving
    Object.keys(newValues).forEach((key) => {
      if (newValues[key] !== "") {
        const valueWithIdentifiers = convertTagsToIdentifiers(newValues[key]);
        updates[key] = valueWithIdentifiers;
      }
    });
