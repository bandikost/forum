...
={(e) => handleImageUpload(e, i)} 
                ...
                <label htmlFor={`file${i2}`}>
                  <div className="image-container">
                    <input
                      id={`file${i2}`}
                      type="file"
                      onChange
={(e) => handleImageUpload(e, i2)} 
                    />
                    <div className="image-now" >
                      Текущее изображение для объекта карточки 2: <div style={{fontWeight: "500", fontSize: "20px", marginLeft: "10px"}}> № {2}</div>
                    </div>{" "} 
                    {objects && objects[`imageUrl${i2}`] && (
                      <div style={{display: "contents",
                        margin: "0 auto 0"}}><img src={objects[`imageUrl${i2}`]} alt="property" /></div>
                    )}
                  </div>
                </label>
              </div>
