...
export const GeneralEdit6 = () => {
  ...
  const [newValues, setNewValues] = useState({
    ...
    isSold6: objects6.isSold6 || false,
  });
  ...
  const handleSoldCheckboxChange = () => {
    const db = getDatabase();
    const objects6Ref = ref(db, "objects6"); // Assuming 'objects3' is the correct reference in your database
    set(objects6Ref, { ...objects6, isSold6: !newValues.isSold6 }); // Update the 'isVisible' property
  };
  ...
  useEffect(() => {
    if (objects6.name1) {
    document.title = objects6.name1 + " - Редактор";
    }
  }, [objects6.name1]);
          ...
          style={{position: "relative", top: "60px", margin: "0 auto 0"}}>
            {[1].map((i) => (
              <div key={i}  className="table-objects6" style={{display: "flex",
                ...
                >
                {objects6 && convertIdentifiersToTags(objects6?.[`minimal${i}`])}
                </textarea>
                </label>
                <label style={{ margin: "0px 50px" }}>
